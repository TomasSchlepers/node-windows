...
var daemon = function(config){
  ...
  Object.defineProperties(this,{
    ...
    /**
     * @property {string} [logOnAccount]
     * If you need to specify a specific user or particular credentials for the service log on as once installed,
     * the following attribute may be helpfull. Both account and password are required if used. If none provided,
     * the service will run as the Local System account.
     */
    logOnAccount: {
      enumerable: false,
      writable: true,
      configurable: false,
      value: config.logOnAccount !== undefined ? config.logOnAccount : null
    },

    /**
     * @property {string} [logOnPassword]
     * If you need to specify a specific user or particular credentials for the service log on as once installed,
     * the following attribute may be helpfull. Both account and password are required if used. If none provided,
     * the service will run as the Local System account.
     */
    logOnPassword: {
      ...
      writable: true,
      configurable: false,
      value: config.logOnPassword !== undefined ? config.logOnPassword : null
    },
    ...
    install: {
      ...
      value: function(dir){
        ...
        if ((this.logOnAccount ^ this.logOnPassword) !== 0) {
          throw Error('Both logOnAccount and logOnPassword are required if applicable.');
        }
        ...
        fs.exists(dir,function(exists){
          ...
          fs.writeFile(path.resolve(dir,me.id+'.xml'),me._xml,function(){
            ...
            winsw.createExe(me.id,dir,function(){
              ...
              const installCmd = `"${path.resolve(dir,me._exe)}" install ${me.logOnAccount ? `--user ${me.logOnAccount} --password ${me.logOnPassword}` : ''}`
